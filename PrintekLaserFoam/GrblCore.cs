...
namespace PrintekLaserFoam
{
	...
	public class GrblCore
	{
		...
		public void RunProgramFromPosition()
		{
			...
			{
				bool homing = false;
				int position = PrintekLaserFoam.RunFromPositionForm.CreateAndShowDialog(LoadedFile.Count, Configuration.HomingEnabled, out homing);
				if (position >= 0)
		...
		private void UserWantToContinue()
		{
			...
			int position = PrintekLaserFoam.ResumeJobForm.CreateAndShowDialog(mTP.Executed, mTP.Sent, mTP.Target, mTP.LastIssue, Configuration.HomingEnabled, homing, out homing, setwco, setwco, out setwco, mTP.LastKnownWCO);
		...
		private GrblVersionInfo StatusReportVersion(string rline)
		{
			...
			//the check of Pin: is due to compatibility with 1.0c https://github.com/arkypita/PrintekLaserFoam/issues/317
		...
		public static string DataPath
		{
			get
			{
				if (mDataPath == null)
				{
					mDataPath = System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "PrintekLaserFoam");
		...
		public static string TempPath
		{
			get
			{
				if (mTempPath == null)
				{
					mTempPath = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "PrintekLaserFoam");
		...
		{ System.Diagnostics.Process.Start(@"http://printek.it/laserfoamhelp/"); }
		...
		public bool ContinuosJogEnabled { get { return (bool)Settings.GetObject("Enable Continuous Jog", true); } }
