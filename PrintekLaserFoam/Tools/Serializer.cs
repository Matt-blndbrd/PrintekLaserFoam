	...
	public class Serializer
	{
		...
		public static bool ObjToFile(object ObjectToSave, string filename, SerializationMode SerializeMode, string Password, bool Compression)
		{
			lock (ThreadLock)
			{
				...
				try
				{
					filename = System.IO.Path.Combine(PrintekLaserFoam.GrblCore.DataPath, filename);
		...
		private class LogDatedWriter
		{
			...
			{
				PrintekLaserFoam.Logger.LogMessage("Serialization", Text);
			}
			...
			{
				WriteLogLines("");
				PrintekLaserFoam.Logger.LogMessage("Serialization", "---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
			}
